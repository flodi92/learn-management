...
export type ResultsOfUser = Map<
  Date,
  { skill: Skill; result: Result; aspects: Aspect[] }
>;
...
type SuggestTestableSkills = (
  targetSkill: Skill,
  userResults: ResultsOfUser,
) => { skill: Skill; priority: Priority; estimatedTime: number };
...
const suggestTestableSkills1: SuggestTestableSkills = (
  targetSkill,
  userResults,
) => {
  const getLearnHierarchy = (
    targetSkill: Skill,
    userResults: ResultsOfUser,
  ) => {
    interface Hierachy {
...
const suggestTestableSkills2: SuggestTestableSkills = (
  targetSkill,
  userResults,
) => {
...
const main = () => {
  ...
  do {
    ...
    const addSkill = prompt(
      `Wähle so lange aus den Vorschlägen aus, bis du die Sitzung hast ${suggestion}`,
    ) as unknown as Skill;
    ...
    console.log(
      sessionInTime()
        ? 'Deine Planung ist noch unter der Zeit'
        : 'Deine Planung ist über der geplanten Zeit',
    );
  ...
  for (const skill of session) {
    const examineLearner = (skill: Skill) =>
      [
        undefined as unknown as Result,
        undefined as unknown as Aspect[],
      ] as const;
